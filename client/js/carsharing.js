var carSharingLayer = {
	...
	getCarBrands : function(callback){
		integreen.getStationDetails('carsharing-ws/rest/cars/',{},displayBrands);	
 	...
 	get:function(){
		...
		positionsLayer.events.on({
				...
				if (!e.feature.cluster){
					...
					integreen.retrieveData(station,"carsharing-ws/rest/",getCarsharingStation);
				}
			}
		});
		function getCarsharingStation(details,current){
			...
			radialProgress($(".carsharingstation .number-available")[0])
				...
				.render();
			integreen.getChildStationsData(details.id,"carsharing-ws/rest/cars/",displayCarsharingData);
