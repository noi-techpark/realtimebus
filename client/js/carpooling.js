var carpoolingLayer = {
		...
		var currentDrawnFeatures;
                positionsLayer.events.on({
					...
					if (e.feature.attributes.stationtype=='Carpoolinghub')
	                                        integreen.retrieveData(station,"carpooling-ws/rest/hubs/",displayHubsData);
					if (e.feature.attributes.stationtype=='CarpoolingUser'){
						drawRouteToDestination(e.feature);
	                        		integreen.retrieveData(station,"carpooling-ws/rest/user/",displayUserData);
					}
                                }else{
		...
		function displayClusterFeatures(features){
			...
			features.forEach(function(feature,index){
				...
				$('.station .content ul .clusterhub'+feature.attributes.stationcode).click(function(){
					integreen.retrieveData(feature.attributes.stationcode,"carpooling-ws/rest/hubs/",displayHubsData);	
				});
				$('.station .content ul .clusteruser'+feature.attributes.stationcode).click(function(){
					drawRouteToDestination(feature);
					integreen.retrieveData(feature.attributes.stationcode,"carpooling-ws/rest/user/",displayUserData);	
				});
			});
                        $('.station').show();
		}
	...
	getTypes : function(callback){
                integreen.getStationDetails('carpooling-ws/rest/hubs/',{},displayBrands);
			...
			integreen.retrieveData("innovie","carpooling-ws/rest/",displayAllData);
